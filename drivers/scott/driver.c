...
static DRIVER_DISPATCH DispatchRead;
static DRIVER_DISPATCH DispatchWrite;
static DRIVER_DISPATCH DispatchIoCtl;
...
DispatchRead(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    ...
    if (!IoStack)
    {
        NtStatus = STATUS_BAD_STACK;
        goto Done;
    }

    NtStatus = HandleIoRead(Irp, IoStack, &Information);
...
DispatchWrite(IN PDEVICE_OBJECT DeviceObject, IN PIRP Irp)
{
    ...
    NtStatus = HandleIoWrite(Irp, IoStack, &Information);
...
DriverEntry(IN PDRIVER_OBJECT DriverObject, IN PUNICODE_STRING RegPath)
{
    ...
    NtStatus = IoCreateDevice(
        DriverObject, 0, &DeviceName, FILE_DEVICE_UNKNOWN, FILE_DEVICE_SECURE_OPEN, FALSE, &DeviceObject);
